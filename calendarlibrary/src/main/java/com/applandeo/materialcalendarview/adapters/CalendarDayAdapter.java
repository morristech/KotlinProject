...
class CalendarDayAdapter extends ArrayAdapter<Date> {
    ...
    private void setLabelColors(TextView dayLabel, Calendar day) {
        ...
        if (!isFilledDayAvailable(day)){
            DayColorsUtils.setDayColors(dayLabel, mCalendarProperties.getDaysLabelsColor(),
                    Typeface.NORMAL, R.drawable.background_color_circle_stroke);
...
//                    R.drawable.background_color_circle_selector);
    ...
    private boolean isFilledDayAvailable(Calendar day) {
        return !mCalendarProperties.getFilledDays().contains(day);
    }
